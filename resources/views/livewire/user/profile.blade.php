        ...
        <div class="font-std mb-10 w-full rounded-2xl bg-white p-10 font-normal leading-relaxed text-gray-900 shadow-xl">
            <div class="flex flex-col md:flex-row">
                <div class="md:w-1/3 text-center mb-8 md:mb-0">
                    {{-- <form>
                        <img id="imagePreview" src="{{ $profile_picture ? asset('storage/' . $profile_picture) : '' }}"
                            alt="Profile Preview"
                        ...
                        <!-- Show upload button only if profile picture is not set -->
                        @if (!$profile_picture)
                            <input type="file" wire:model="image" id="fileInput" accept="image/*" class="hidden" />
                            <button type="button" onclick="openFilePicker()" wire:click.prevent="uploadProfilePicture"
                    ...
                    <script>
                        ...
                        // Function to handle file upload and show the preview
                        document.getElementById('fileInput').addEventListener('change', function(event) {
                            const file = event.target.files[0];
                            const imagePreview = document.getElementById('imagePreview'); // Get image preview element

                            if (file) {
                                const reader = new FileReader();
                                reader.onload = function(e) {
                                    imagePreview.src = e.target.result; // Set image preview to the loaded file
                                };
                    ...
                    @if (!$current_valid_id)
                        <div x-data="{ openFilePicker() { $refs.fileInput.click() } }" class="pt-12">

                            <img id="imagePreview"
                                src="{{ $profile_picture ? asset('storage/' . $profile_picture) : '' }}"
                                alt="Profile Preview"
                                class="rounded-lg w-12 h-12 mx-auto mb-4 border-4 border-blue transition-transform duration-300 hover:scale-105 ring ring-gray-300">

                            <input type="file" wire:model="valid_id" x-ref="fileInput" accept="image/*"
                                class="hidden" />
                            <button type="button" @click.prevent="openFilePicker" wire:click.prevent="uploadValidID"
                                class="mt-4 bg-blue text-white px-2 py-1 text-xs rounded-lg hover:bg-blue-900 transition-colors duration-300 ring ring-gray-300 hover:ring-indigo-300">
                                Upload Valid ID
                            </button>
                        </div>
                        ...
                        @if (!$current_valid_id)
                            ...
                            <div class="pt-6">
                                <img id="validIDPreview"
                                    src="{{ $current_valid_id ? asset('storage/' . $current_valid_id) : '' }}"
                                    alt="Valid ID"
                ...
                <div class="md:w-2/3 md:pl-8">
                    <div class="md:w-2/3 md:pl-8">
                        <h1 class="text-2xl font-bold text-blue mb-2">{{ $first_name }} {{ $last_name }}</h1>
